...
void calc_sources(int nx, int ny, double *x, double *y, double *dx, double *dy, double **T, double **b)
{
  ...
  for(i=0; i<nx; i++)
   for(j=0; j<ny; j++)
   {
     b[i][j] = -2*cos(PI*y[j]) + PI*PI*x[i]*(x[i]-1)*cos(PI*y[j]);  // set source function here
...
void get_coeffs(int nx, int ny, double *x, double *y, double *dx, double *dy, double **aP, double **aE, double **aW, double **aN, double **aS, double **b, double **T, double **kdiff, double **bcleft, double **bcrght, double **bctop, double **bcbot)
{
  ...
  for(i=1; i<nx-1; i++)
   for(j=1; j<ny-1; j++)
   {
     ...
     aE[i][j] = -1.0;   aW[i][j] = -1.0;
     aN[i][j] = -hxhy_sq;   aS[i][j] = -hxhy_sq;
   }
  ...
  for(i=1; i<nx-1; i++)
   {
     ...
     aP[i][j] = 2.0 * dy[0] * hxhy_sq * qj - 2.0 * (1.0 + hxhy_sq) * pj;
     aW[i][j] = pj;   aE[i][j] = pj;
     aN[i][j] = 2.0 * pj * hxhy_sq;   aS[i][j] = 0.0;
     b[i][j] = (-b[i][j] * pj) + 2.0 * dy[0] * rj * hxhy_sq;
   }
  ...
  for(i=1; i<nx-1; i++)
   {
     ...
     aP[i][j] = - 2.0 * (1.0 + hxhy_sq) * pj - 2.0 * dy[0] * qj * hxhy_sq;
     aW[i][j] = pj;   aE[i][j] = pj;
     aN[i][j] = 0.0;   aS[i][j] = 2.0 * pj * hxhy_sq;
     b[i][j] = (-b[i][j] * pj) - 2.0 * dy[0] * rj * hxhy_sq;
   }
