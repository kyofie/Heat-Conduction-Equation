...
void calc_sources(int nx, int ny, double *x, double *y, double *dx, double *dy, double **T, double **b)
{
  ...
  for(i=0; i<nx; i++)
   for(j=0; j<ny; j++)
   {
     b[i][j] = -((1 + 0.8 * x[i] * (1-x[i]) * cos(PI*y[j])) * (2 * cos(PI * y[j]) - x[i] * (x[i] - 1) * PI * PI * cos(PI*y[j])) - 0.8 * ((1 - 2 * x[i] * cos(PI * y[j])) *  (1 - 2 * x[i] * cos(PI * y[j])) + (x[i] * (x[i] - 1) * PI * sin(PI * y[j])) * (x[i] * (x[i] - 1) * PI * sin(PI * y[j]))));                      // set source function here
...
void get_coeffs(int nx, int ny, double *x, double *y, double *dx, double *dy, double **aP, double **aE, double **aW, double **aN, double **aS, double **b, double **T, double **kdiff, double **bcleft, double **bcrght, double **bctop, double **bcbot)
{
  ...
  for(i=1; i<nx-1; i++)
   for(j=1; j<ny-1; j++)
   {
     double kdiffy = (kdiff[i][j+1]-kdiff[i][j-1])/(2*dy[0]);
     double kdiffx = (kdiff[i+1][j]-kdiff[i-1][j])/(2*dx[0]);
     aP[i][j] = 2.0 * kdiff[i][j] * (1 + hxhy_sq);
     aE[i][j] = - kdiff[i][j] + (dx[0] * kdiffx)/2;   aW[i][j] = - kdiff[i][j] + (dx[0] * kdiffx)/2;
     aN[i][j] = -hxhy_sq * (kdiff[i][j] + (kdiffy* dy[0])/2);   aS[i][j] = -hxhy_sq * (kdiff[i][j] - (kdiffy * dy[0])/2);
   }
  ...
  i = 0;
   for(j=0; j<ny; j++)
   {
     ...
     if(j == 0){
       //forward difference
       kdiffy = (kdiff[i][j+1]-kdiff[i][j])/dy[0];
     }
     else if(j == ny-1){
       //backward differnce
       kdiffy = (kdiff[i][j]-kdiff[i][j-1])/dy[0];
     }
     else{
       kdiffy = (kdiff[i][j+1]-kdiff[i][j-1])/(2*dy[0]);
     }
     //forward difference
     double kdiffx = (kdiff[i+1][j]-kdiff[i][j])/dx[0];
  
     aP[i][j] = 2.0 * kdiff[i][j] * (1 + hxhy_sq) * pj - (kdiff[i][j] - kdiffx * (dx[0]/2))*(2 * dx[0] * qj);
     aE[i][j] = -2.0 * pj *(kdiff[i][j] + kdiffx * (dx[0]/2));       aW[i][j] = 0.0;
     aN[i][j] = -hxhy_sq * pj *(kdiff[i][j] + kdiffy * (dy[0]/2));   aS[i][j] = -hxhy_sq * pj * (kdiff[i][j] - kdiffy * (dy[0]/2));
     b[i][j] = b[i][j] * pj - 2.0 * dx[0] * rj * (kdiff[i][j]-kdiffx * (dx[0]/2));
   }
  ...
  i = nx-1;
   for(j=0; j<ny; j++)
   {
     ...
     //backward difference
     double kdiffx = (kdiff[i][j]-kdiff[i-1][j])/dy[0];
     aP[i][j] = 2.0 * kdiff[i][j] * (1 + hxhy_sq) * pj + (kdiff[i][j] + kdiffx * (dx[0]/2))*(2 * dx[0] * qj);
     aW[i][j] = 2.0 * kdiff[i][j] * pj;   aE[i][j] = 0.0;
     aN[i][j] = -hxhy_sq * pj *(kdiff[i][j] + kdiffy * (dy[0]/2));   aS[i][j] = -hxhy_sq * pj * (kdiff[i][j] - kdiffy * (dy[0]/2));
     b[i][j] = b[i][j] * pj + 2.0 * dx[0] * rj * (kdiff[i][j] + kdiffx * (dx[0]/2));
   }
  ...
  for(i=1; i<nx-1; i++)
   {
     ...
     //forward difference
     double kdiffy = (kdiff[i][j+1]-kdiff[i][j])/dx[0];
     double kdiffx = (kdiff[i+1][j]-kdiff[i-1][j])/(2*dy[0]);
     aP[i][j] = 2.0 * kdiff[i][j] * (1 + hxhy_sq) * pj - hxhy_sq * (kdiff[i][j] - kdiffy * (dy[0]/2)) * 2.0 * dy[0] *qj;
     aW[i][j] = - pj * (kdiff[i][j] + kdiffy * (dx[0]/2));   aE[i][j] = - pj * (kdiff[i][j] - kdiffy * (dx[0]/2));
     aN[i][j] = -2.0 * hxhy_sq * kdiff[i][j] * pj;   aS[i][j] = 0.0;
     b[i][j] = b[i][j] * pj - hxhy_sq * (kdiff[i][j] - kdiffy * (dy[0]/2)) * 2.0 * dy[0] * rj;
   }
  ...
  for(i=1; i<nx-1; i++)
   {
     ...
     //backward difference
     double kdiffy = (kdiff[i][j]-kdiff[i][j-1])/dx[0];
     double kdiffx = (kdiff[i+1][j]-kdiff[i-1][j])/(2*dy[0]);
     aP[i][j] = 2.0 * kdiff[i][j] * (1 + hxhy_sq) * pj + hxhy_sq * (kdiff[i][j] + kdiffy * (dy[0]/2)) * 2.0 * dy[0] *qj;
     aW[i][j] = - pj * (kdiff[i][j] + kdiffy * (dx[0]/2));   aE[i][j] = - pj * (kdiff[i][j] - kdiffy * (dx[0]/2));
     aN[i][j] = 0.0;   aS[i][j] = -2.0 * hxhy_sq * kdiff[i][j] * pj;
     b[i][j] = b[i][j] * pj + hxhy_sq * (kdiff[i][j] + kdiffy * (dy[0]/2)) * 2.0 * dy[0] * rj;
   }
...
void solve_gssor(int nx, int ny, double *x, double *y, double *dx, double *dy, double **aP, double **aE, double **aW, double **aN, double **aS, double **b, double **T, double **Tpad, double **Tpnew, int max_iter, double tol, double relax_T, double **kdiff, double **bcleft, double **bcrght, double **bctop, double **bcbot)
{
  ...
  double **Tprev;
  Tprev = (double **)malloc(nx * sizeof(double *));
  for(i=0; i<nx; i++) Tprev[i] = (double *)malloc(ny * sizeof(double));
 ...
 // now perform iterations
  ...
  for(i=0; i<nx; i++)
   ...
   if(get_l2err_norm(nx, ny, Tprev, T)>tol){
   	get_coeffs(nx, ny, x, y, dx, dy, aP, aE, aW, aN, aS, b, T, kdiff, bcleft, bcrght, bctop, bcbot);
   	solve_gssor(nx, ny, x, y, dx, dy, aP, aE, aW, aN, aS, b, T, Tpad, Tpnew, max_iter, tol, relax_T, kdiff, bcleft, bcrght, bctop, bcbot);
   	 
   }


  for(i=0; i<nx; i++) free(Tprev[i]);
  free(Tprev);
}
...
void output_soln(int nx, int ny, int iter, double *x, double *y, double **T, double **Tex)
{
  ...
  sprintf(fname, "T_xy_vark_%03d_%03d_%04d.dat", nx, ny, iter);
...
int main()
{
  ...
  // ---
     ...
     solve_gssor(nx, ny, x, y, dx, dy, aP, aE, aW, aN, aS, b, T, wrk1, wrk2, max_iter, tol, relax_T, kdiff, bcleft, bcrght, bctop, bcbot);
